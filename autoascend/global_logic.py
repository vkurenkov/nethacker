    ...
    def current_strategy(self):
        ...
        while 1:
            ...
            if self.milestone == Milestone.BE_ON_FIRST_LEVEL:
                # hypothesis: descend early when low on health, while keeping
                # the usual XP target for characters that can safely farm.
                condition = lambda: self.agent.blstats.experience_level >= 8 or (
                    self.agent.blstats.experience_level >= 6 and
                    self.agent.blstats.hitpoints <= 0.35 * self.agent.blstats.max_hitpoints)
